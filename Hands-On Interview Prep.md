...
- **The red flag:** Inbound rule with `0.0.0.0/0` on port 22 (SSH) or 3389 (RDP). This is the most common misconfiguration they'll test. In the incident scenario, an open port may be the entry point. Remote access via these ports should not be necessary in an AWS environment. In such a case, the customer should be guided to use AWS SSM instead (which is more secure and easier to audit).
